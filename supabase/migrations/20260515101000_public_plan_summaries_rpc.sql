-- =====================================================================
-- I4 — Exposição segura de planos.
--
-- Antes, `plans` tinha política pública de SELECT para planos ativos, o
-- que expunha colunas internas como IDs/estado de sincronização Pagar.me.
-- Agora o público consome apenas a RPC sanitizada abaixo; acesso direto
-- à tabela fica restrito às políticas de super admin já existentes.
-- =====================================================================
...
DROP POLICY IF EXISTS "Public can view features of active plans" ON public.plan_features;
...
COMMENT ON FUNCTION public.get_public_plan_summaries() IS
  'Lista pública sanitizada de planos ativos, sem IDs ou estado de integração Pagar.me.';
...
BEGIN
  ...
  SELECT COALESCE(jsonb_agg(
    jsonb_build_object(
      ...
      'has_pagarme_subscription', s.pagarme_subscription_id IS NOT NULL,
...
COMMENT ON FUNCTION public.get_my_subscription_summaries(uuid) IS
  'Resumo de assinaturas do restaurante sem expor IDs Pagar.me ao cliente.';
