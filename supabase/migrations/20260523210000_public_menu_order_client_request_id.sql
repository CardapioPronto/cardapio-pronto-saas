-- Idempotência do checkout público via client_request_id (evita pedido duplicado em retry PIX)

ALTER TABLE public.orders
  ADD COLUMN IF NOT EXISTS client_request_id text;

CREATE UNIQUE INDEX IF NOT EXISTS idx_orders_restaurant_client_request
...
COMMENT ON FUNCTION public.create_public_menu_order(jsonb) IS
  'Cria pedido público; suporta client_request_id para idempotência em retry de pagamento online.';
