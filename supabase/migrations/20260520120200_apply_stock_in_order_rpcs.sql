-- =====================================================================
-- Estoque opcional — Bloco D (integração com pedidos)
--
-- Roteiro: docs/ROTEIRO_ESTOQUE_OPCIONAL.md
-- Branch:  controle-estoque-opcional
-- Depende de: 20260520120100_create_stock_control_rpcs.sql
--
-- Reescreve três RPCs existentes para aplicar/estornar estoque na mesma
-- transação:
--
--   * create_pos_order(jsonb)              — após inserir itens, baixa.
--                                            Aceita allow_negative_override
--                                            por gestor (com reason).
--   * create_public_menu_order(jsonb)      — após inserir itens, baixa.
--                                            Sem override (cliente final).
--   * update_order_status(uuid, text)      — viabiliza a reabertura
--                                            (cancelado/finalizado →
--                                            pendente) com revalidação de
--                                            estoque. Cancelamento e
--                                            pagamento_falhou estornam.
--
-- Trade-off: cada RPC é redefinida por inteiro (CREATE OR REPLACE),
-- mantendo o padrão histórico do repositório (cada iteração nova é uma
-- migration completa). Diferenças em relação à versão anterior estão
-- comentadas com "-- [estoque]".
-- =====================================================================

-- =====================================================================
-- D1. create_pos_order
-- Versão base: 20260509120000_create_pos_order_transaction_rpcs.sql.
-- Diferenças marcadas com "-- [estoque]".
-- =====================================================================
...
DECLARE
  ...
  v_order_id uuid;
  v_order_number text;
  -- [estoque] override negativo autorizado por gestor.
...
BEGIN
  ...
  -- [estoque] flag e motivo do override.
  ...
  -- [estoque] override exige permissão de gestão de produtos + motivo.
  ...
  -- [estoque] baixa após itens existirem (RPC do Bloco C). Se houver
  -- falta de saldo e não houver override, lança exceção e a transação
  -- toda volta atrás.
...
-- =====================================================================
-- D2. create_public_menu_order
-- Versão base: 20260518000100_create_public_menu_order_rate_limit.sql.
-- Único acréscimo: PERFORM apply_stock_for_order após inserir itens.
-- Sem override: o cliente final nunca pode forçar venda sem saldo.
-- =====================================================================
...
BEGIN
  ...
  -- [estoque] baixa após inserir os itens. Sem override no canal público.
...
COMMENT ON FUNCTION public.create_public_menu_order(jsonb) IS
  'Cria pedido público com promoções/cupons server-side, rate limit 10/min por IP, e baixa de estoque opcional via apply_stock_for_order.';
...
--
-- Mudanças:
--   * Permite reabrir pedido (cancelado/finalizado → pendente). Antes
--     bloqueava com EXCEPTION. A UI já oferecia a ação no histórico,
--     mas a RPC negava — esse desalinhamento agora some.
--   * Em transição para 'cancelado' ou 'pagamento_falhou', estorna
--     estoque via revert_stock_for_order.
--   * Em transição para 'pendente' vinda de 'cancelado'/'finalizado',
--     re-aplica via apply_stock_for_order. Se faltar saldo, a RPC
--     levanta exceção e o status NÃO muda.
-- =====================================================================
...
BEGIN
  ...
  -- [estoque] reabertura: cancelado/finalizado → pendente é um caminho
  -- explicitamente permitido. Outras saídas a partir desses estados
  ...
  v_allowed_next_statuses := CASE v_order.status
    ...
    WHEN 'finalizado' THEN ARRAY['pendente']
    ELSE ARRAY[]::text[]
  END;
  ...
  v_is_reopen := v_order.status IN ('cancelado', 'finalizado') AND p_status = 'pendente';
  v_is_revert := p_status IN ('cancelado', 'pagamento_falhou');

  -- [estoque] aplica/estorna ANTES de mudar o status. Falha no estoque
  -- (ex.: reabrir sem saldo) levanta exceção e o pedido permanece como
  -- estava — a UI exibe o erro vindo da RPC.
  IF v_is_reopen THEN
    PERFORM public.apply_stock_for_order(v_order.id, false);
  ELSIF v_is_revert THEN
    PERFORM public.revert_stock_for_order(v_order.id);
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'table_status', CASE
      ...
      ELSE 'livre'
    END,
    'reopened', v_is_reopen,
    'reverted_stock', v_is_revert
  );
END;
$$;
...
COMMENT ON FUNCTION public.update_order_status(uuid, text) IS
  'Transição de status do pedido. Cancelado/finalizado → pendente é permitido (revalida estoque). Cancelar/pagamento_falhou estornam estoque.';
