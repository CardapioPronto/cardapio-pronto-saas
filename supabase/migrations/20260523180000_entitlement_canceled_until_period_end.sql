-- Acesso até o fim do período já pago após cancelamento (padrão SaaS).
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'status', CASE
      ...
      THEN 'trialing'
      WHEN v_subscription.status = 'canceled'
        AND COALESCE(v_subscription.current_period_end, NULL) IS NOT NULL
...
COMMENT ON FUNCTION public.get_restaurant_subscription_entitlement(uuid) IS
  'Entitlement: trial/ativo/past_due/pending; cancelado mantém acesso até current_period_end ou fim do trial.';
