-- Trials cancelados por checkout incompleto (ex.: PIX falhou após cancelar o trial antigo)
-- voltam a trialing enquanto trial_ends_at estiver no futuro e não houver outra assinatura vigente.
...
WHERE s.status = 'canceled'
  ...
  AND NOT EXISTS (
    ...
    WHERE newer.restaurant_id = s.restaurant_id
      AND newer.id <> s.id
      AND newer.status IN ('active', 'trialing', 'pending', 'past_due')
  );
...
BEGIN
  ...
  ORDER BY
    CASE
      ...
      WHEN s.status = 'canceled' AND s.trial_ends_at > now() THEN 4
      ELSE 9
    END,
    s.created_at DESC
  LIMIT 1;
