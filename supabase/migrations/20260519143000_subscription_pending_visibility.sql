-- Assinaturas com boleto pendente: expor status `pending` na listagem do dono
-- (sem alterar entitlement — pending não libera acesso ao produto).
...
BEGIN
  ...
  WHERE s.restaurant_id = p_restaurant_id
    AND s.status IN ('active', 'trialing', 'past_due', 'pending', 'canceled');
...
COMMENT ON FUNCTION public.get_my_subscription_summaries(uuid) IS
  'Resumo de assinaturas do restaurante (inclui pending para boleto aguardando pagamento).';
