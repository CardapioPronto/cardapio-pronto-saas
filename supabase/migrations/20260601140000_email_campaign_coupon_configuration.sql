-- Bloco 3: configurable campaign coupons.
...
BEGIN
  ...
  IF v_campaign.coupon_id IS NOT NULL THEN
    ...
    IF FOUND THEN
      ...
      SET title = 'Cupom da campanha: ' || left(v_campaign.name, 80),
          description = 'Gerado automaticamente para campanha de e-mail.',
          discount_type = v_discount_type,
          discount_value = v_discount_value,
          valid_until = now() + make_interval(days => v_valid_days),
...
COMMENT ON FUNCTION public.generate_email_campaign_coupon(uuid, text, numeric, integer, numeric) IS
  'Gera ou atualiza um cupom rastreavel configuravel vinculado a uma campanha de e-mail.';
