-- Bloco 7: promoções aplicadas server-side e visíveis no menu público.
-- Regras:
--  1. Promoção de produto/categoria sempre aplica por linha (melhor desconto vence).
--  2. Promoção de pedido e cupom não somam: vale o maior; o outro é descartado.
--  3. Quando o cupom é descartado, ele não é consumido (sem INSERT em coupon_usage).
--  4. Promoções inativas, fora da validade ou abaixo do `min_order_value` não entram.
...
BEGIN
  ...
  -- Primeira passada: calcula subtotal e resolve promoções por linha.
  ...
  -- Melhor promoção de pedido aplicável ao subtotal pós-promoções de item.
  ...
  -- Cupom: valida em qualquer cenário, mas só é consumido se for o melhor desconto de pedido.
  ...
  -- Regra de convivência: promoção de pedido e cupom não somam; vale o maior.
...
-- RPC pública para o cardápio listar promoções ativas
CREATE OR REPLACE FUNCTION public.get_public_restaurant_promotions(p_restaurant_id uuid)
RETURNS jsonb
LANGUAGE plpgsql
STABLE
SECURITY DEFINER
  ...
  RETURN COALESCE(
    (
      SELECT jsonb_agg(
        ...
        ORDER BY p.applicable_to, p.discount_value DESC
      )
      FROM public.promotions p
      WHERE p.restaurant_id = p_restaurant_id
        AND COALESCE(p.is_active, false) = true
        AND v_now >= p.valid_from
        AND (p.valid_until IS NULL OR v_now <= p.valid_until)
    ),
    '[]'::jsonb
  );
END;
...
GRANT EXECUTE ON FUNCTION public.get_public_restaurant_promotions(uuid) TO anon, authenticated;
