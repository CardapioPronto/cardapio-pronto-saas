-- Bloco 3: birthday audience for campaign automation.
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      profile.birth_date,
      bday.birthday_next_at,
      c.created_at
    FROM contact_base c
    LEFT JOIN public.crm_customer_profiles profile
      ON profile.restaurant_id = c.restaurant_id
     AND profile.phone_normalized = c.phone_normalized
    LEFT JOIN LATERAL (
      SELECT
    ...
    LEFT JOIN LATERAL (
      WITH raw AS (
        SELECT (
          ...
          + (profile.birth_date - date_trunc('year', profile.birth_date)::date)
        )::date AS birthday_this_year
      )
      SELECT CASE
        WHEN profile.birth_date IS NULL THEN NULL
        WHEN birthday_this_year < current_date THEN (birthday_this_year + interval '1 year')::date
        ELSE birthday_this_year
      END AS birthday_next_at
      FROM raw
    ) bday ON true
  ),
  filtered AS (
    ...
    WHERE CASE v_type
      ...
      WHEN 'birthday' THEN birth_date IS NOT NULL
        AND birthday_next_at >= current_date
        AND birthday_next_at < current_date + make_interval(days => v_days)
      ELSE true
    END
  ),
  ranked AS (
    ...
    FROM filtered
    ORDER BY
      CASE WHEN v_type = 'birthday' THEN birthday_next_at ELSE NULL END ASC NULLS LAST,
  ...
  SELECT COALESCE(
    jsonb_agg(
      jsonb_build_object(
        ...
        'birthday_next_at', birthday_next_at
      )
    ),
    '[]'::jsonb
  )
...
COMMENT ON FUNCTION public.get_email_campaign_recipients(uuid, jsonb, integer) IS
  'Seleciona destinatarios de campanhas por comportamento, categoria comprada, aniversario, opt-in e descadastro.';
