...
function buildPlanBody(
  ...
  return {
    ...
    payment_methods: ["credit_card", "pix", "debit_card"],
    installments: [1],
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: plan, error: planErr } = await admin
      .from("plans")
      .select(
        "id, name, description, price_monthly, price_yearly, trial_days, is_active, pagarme_plan_id_monthly, pagarme_plan_id_yearly",
      )
      .eq("id", plan_id)
      .maybeSingle();
