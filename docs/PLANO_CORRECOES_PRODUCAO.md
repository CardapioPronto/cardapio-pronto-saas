...
- [x] Validar que erros React e Edge chegam ao Sentry. (2026-05-14: React/frontend validado no Sentry pelo `Error Monitor` com eventos `ReferenceError`/`TypeError` de bundle/componentes; Edge validado com evento `runtime:supabase_edge`, ambiente `production`, nível `error`, originado por Edge Function.)
- [x] Configurar alertas para Edge Function error rate. (2026-05-14: criado Sentry Metric Monitor `Edge Functions errors - production` no projeto `javascript-react`, ambiente `production`, métrica `Number of Errors`, intervalo 5 min, filtro `runtime:supabase_edge level:error`, prioridade alta acima de 0 e alerta por e-mail. Observação: alerta percentual de error rate depende de transações/métricas de requisições totais nas Edge Functions.)
